...
describe("diagnostics-otel service", () => {
  ...
  test("replaces signal-qualified shared endpoint for metrics and logs", async () => {
    const service = createDiagnosticsOtelService();
    ...
    expect(traceOptions.url).toBe("https://collector.example.com/v1/traces");
    expect(metricOptions.url).toBe("https://collector.example.com/v1/metrics");
    expect(logOptions.url).toBe("https://collector.example.com/v1/logs");
    await service.stop?.(ctx);
  });

  test("replaces signal-qualified shared endpoint for multiple signals with query params", async () => {
    const service = createDiagnosticsOtelService();
